...
def train_autoenc(dataf):
    ...
    while epoch < 200:
        ...
        dump_batch(fakes, 4, "tmp/fakes.png")
        dump_batch(reals, 4, "tmp/reals.png")
...
def train_gan(dataf):
    ...
    logger = CSVLogger('tmp/loss.csv')  # yeah, you can use callbacks independently
...
def run_batches(gen, disc, gan, faces, logger, itr_generator):
    ...
    for batch in itr_generator:
        ...
        fakes, noises = sample_fake(gen)
        reals = sample_faces(faces)
        print("fakes[0]:", len(fakes[0]))
        print("reals[0]:", len(reals[0]))
        # Add noise...
...
def end_of_batch_task(batch, gen, disc, reals, fakes):
    try:
        ...
        # Animation dump
        dump_batch(reals, 4, "tmp/reals.png")
        dump_batch(fakes, 4, "tmp/fakes.png")  # to check how noisy the image is
        frame = gen.predict(_bits)
        animf = os.path.join(Args.anim_dir, "tmp/frame_{:08d}.png".format(int(batch / 10)))
        dump_batch(frame, 4, animf)
        dump_batch(frame, 4, "tmp/frame.png")
